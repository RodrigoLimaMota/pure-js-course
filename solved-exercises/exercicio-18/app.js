...
const removeSubmitParagraph = () => {
  const submitFeedbackExist = document.querySelector('[data-feedback="submit-feedback"]')

  if (submitFeedbackExist) {
...
const showUserInfo = event => {
  ...
  if (!isUsernameValid) {
    insertParagraphIntoDOM(invalidUserInfo)
    return
  }
  insertParagraphIntoDOM(validUsertInfo)
}
...
inputUsername.addEventListener('input', showUserInfo)
