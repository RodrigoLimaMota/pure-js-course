...
const newRandomNumbers = randomNumbers.filter(randomNumber => (randomNumber % 2) !== 0)
...
const amountAbove501 = crazyNumbers.reduce((accumulator, item) => {
  if (item < 501) {
    accumulator++
  }
  return accumulator
}, 0)
console.log(amountAbove501)
...
const moviesAbove2000 = tarantinoMovies.filter(({ release }, index) => {
  isBeforeTheYear2000 = release < 2000
  if (isBeforeTheYear2000) {
    return tarantinoMovies[index]
  }
})

console.log(moviesAbove2000)
...
const onlySeriesNames = tvShows.map(serie => serie.name)
console.log(onlySeriesNames)
/*
  07
