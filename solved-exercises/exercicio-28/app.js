...
request.addEventListener(`readystatechange`, () => {
  ...
  const isRequestOk = readyState === 4 && status === 200
  const isRequestNotOk = readyState === 4
...
personalInformations.incrementAge = () => {
  return personalInformations.age++
}

for (let i = 0; i <= 5; i++) {
  personalInformations.incrementAge()
}
// console.log(myPersonalInformations)
...
personalInformations.toWalk = metersAmount => {
  personalInformations.travelledDistance += metersAmount
  personalInformations.isWalking = true
}

const valuesInMeter = [3, 2, 34, 560]

valuesInMeter.forEach(value => personalInformations.toWalk(value))

console.log(personalInformations.travelledDistance
  , personalInformations.isWalking)
...
const getSingularOrPlural = (quantity, singular, plural) => quantity === 1 ? singular : plural

personalInformations.logMessage = () => {
  let {
    name,
    age,
    gender,
  ...
  const isFeminine = gender.toLowerCase() === `feminino`
  const getArticle = isFeminine ? `a` : `o`

  const getMetersString = getSingularOrPlural(travelledDistance, `metro`, `metros`)
  const getAgeString = getSingularOrPlural(age, `ano`, `anos`)

  return `Oi. Eu sou ${getArticle} ${name},tenho ${age} ${getAgeString}, ${height} metros de altura, peso ${weight} quilos e, só hoje, eu já caminhei ${travelledDistance} ${getMetersString}.`
}

console.log(personalInformations.logMessage())
...
const falsyValues = [0, NaN, undefined, false, ``, null]
const truthyValues = [1, "rodrigo", {}, [], true, () => { }]

const logBooleanAnalysis = (array) => {
  array.forEach(item => console.log(isTruthy(item)))
}

logBooleanAnalysis(falsyValues)
logBooleanAnalysis(truthyValues)
